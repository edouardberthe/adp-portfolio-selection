...
def PLADP(obj, R: np.ndarray, h_plus: np.ndarray, V: PWLinearValueFunction):
    ...
    I = logical_and(l > 0, h_tidle > 0)    # Sell assets
    if I.any():
        y[I] = h_tidle[I]
        h[0] += (1-theta) * y[I].sum()
    ...
    # Step 2
    while True:
        j_star_idx = logical_and(k > 0, h_tidle < w_tidle)
        if j_star_idx.any():
...
class PWLADPInspectionModel(LADPInspectionModel):
    ...
    def solve(self, R: np.ndarray, h_plus: np.ndarray, V: PWLinearValueFunction):
