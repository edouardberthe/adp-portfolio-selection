...
from adp.parameters import S, T, repeat
...
class ValueFunctionAnimation(FuncAnimation):
    ...
    def __init__(self, updater):
        ...
        self.lines = []
        self.lines.append(self.ax.plot(self.xs, 0 * self.ys, zeros(T), label='Cash', color=colors[0])[0])
        for i in range(1, N):
            self.lines.append(self.ax.plot(self.xs, i * self.ys, zeros(T), label=Data.columns[i-1],
                                           color=colors[i])[0])
        self.ax.legend()
        super().__init__(fig, self.run, S-1, blit=True, repeat=False)
